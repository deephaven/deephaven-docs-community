...
result = source.agg_by([agg.std(cols=["Std = Number"]), agg.median(cols=["Median = Number"])], by=["X"])
```
...
- [Pydoc](https://deephaven.io/core/pydoc/code/deephaven.agg.html?highlight=std#deephaven.agg.std)
