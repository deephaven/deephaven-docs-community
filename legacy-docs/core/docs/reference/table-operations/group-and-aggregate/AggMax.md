...
result = source.agg_by([agg.max_(cols=["MaxNumber = Number"]), agg.last(cols=["LastNumber = Number"])], by=["X"])
```
...
- [Pydoc](https://deephaven.io/core/pydoc/code/deephaven.agg.html?highlight=max#deephaven.agg.max_)
